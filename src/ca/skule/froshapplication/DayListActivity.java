...
public class DayListActivity extends Activity {
	...
	public Day initDay(Day day){
		...
		if (day.getDay().equalsIgnoreCase("Monday"))
		{
			...
			day.addEvent(new Event("Downtown Walkaround", "3:00PM-5:30PM", new Location (""), "The epic stats here. Imaging about 1100 purple F!rosh and leedurs crashing through the streets of downtown Toronto, screaming cheers and just generally being, well, epc! Thats going to be you. Get excited!", Event.FROSH));
			day.addEvent(new Event("Dinner", "5:30PM-6:30PM", new Location ("BA"), "Food! Yum. Need we say more?", Event.FROSH));
			day.addEvent(new Event("EEAT Tutorial", "6:30PM-7:30PM", new Location ("MS"), "The Engineering Entrance Aptitude Test (EEAT) will be administered on September 3 from 9-10:30AM. This tutorial will prepare you with everything you need to know about the test on Tuesday.", Event.FACULTY));
			...
			day.addEvent(new Event("Movie Night", "7:30PM-Late", new Location ("BA"), "Not feeling design-y? Happening at the same time is a movie! Relax and enjoy with your newfound friends.", Event.FROSH));	
		...
		else if (day.getDay().equalsIgnoreCase("Wednesday"))
		{
			...
			day.addEvent(new Event("Department Lunch", "12:00PM-2:00PM", new Location ("BA"), "Get to know your own department. Enjoy free lunch. And get valuable advice for your university life here.", Event.FACULTY));
			day.addEvent(new Event("Choice: Faculty Presentation", "2:00PM-4:00PM", new Location (""), "Content to be determined.", Event.FACULTY));
			day.addEvent(new Event("Choice: World Record Ninja!", "2:00PM-4:00PM", new Location ("FC"), "Feel like breaking a world record? We'd like to tell you about this event. We will gather as many people to play the game \"Ninja\"; more than the current Guinness World Record.", Event.FROSH));
			day.addEvent(new Event("Choice: Skule Nite 101", "2:00PM-4:00PM", new Location ("SF"), "Skule Nite, the Engineering musical and sketch comedy revue, invites you to embark on a journey towards ultimate F!rosh victory! But beware, the quest is fraught with a variety of challenges to test the skills, courage, and creativity of those F!rosh brave enough to undertake it. In the end, a lone group shall emerge victorious, and to the victors go the spoils!", Event.FROSH));
			...
			day.addEvent(new Event("F!rosh Nite at Guvernment", "8:00PM-Late", new Location ("SF"), "This all-ages club night is THE hottest event on campus during Orientation. Held at the Guvernment nightclub, this is free with F!rosh Week admission. A bus will depart from SF", Event.FROSH));
		}
		else if (day.getDay().equalsIgnoreCase("Thursday"))
		{
			day.addEvent(new Event("Classes", "Start from 9:00AM", new Location (""), "I can already hear you booing. But class is important! And you don't want to miss your first lectures", Event.FROSH));
			day.addEvent(new Event("Blue & Gold Bed Races", "3:00PM-5:00PM", new Location ("FC"), "Use whatever you have to build a vehicle that resembles something you can sleep on. Then race against the other colleges to defend the engineering honour!", Event.FROSH));
			day.addEvent(new Event("Suds!", "5:00PM-Late", new Location ("SF"), "", Event.FROSH));
		}
